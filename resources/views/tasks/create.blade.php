...
<div class="max-w-xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Nouvelle tâche</h1>

    {{-- ERRORS --}}
    @if($errors->any())
    <div class="flex gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-6">
        <svg class="w-5 h-5 flex-shrink-0 mt-0.5 text-red-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <form method="POST" action="{{ route('tasks.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          ...
                          class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition resize-none {{ $errors->has('description') ? 'border-red-400 bg-red-50' : '' }}">{{ old('description') }}</textarea>
            </div>
                ...
                <div>
                    ...
                    <select name="priority" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition">
                        @foreach(['Nulle','Basse','Normale','Moyenne','Haute','Critique'] as $i => $label)
                            <option value="{{ $i }}" {{ old('priority', 0) == $i ? 'selected' : '' }}>{{ $i }} — {{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
